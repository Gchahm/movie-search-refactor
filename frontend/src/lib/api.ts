...
function validateMovie(movie: Movie) {
  if (!movie || typeof movie !== 'object') {
    throw new Error('Movie payload is required');
  }
  assertNonEmpty(movie.title, 'title');
  assertNonEmpty(movie.imdbID, 'imdbID');
  if (!Number.isFinite(movie.year)) {
    throw new Error('year must be a number');
  }
  if (typeof movie.poster !== 'string') {
    throw new Error('poster must be a string');
  }
}
