        ...
        describe('should return 400', () => {
            it('when query parameter is missing', () => {
                return request(app.getHttpServer())
                    ...
                    .expect(400)
                    .expect((res) => {
                        expect(Array.isArray(res.body.message) ? res.body.message : [res.body.message]).toEqual(
                            expect.arrayContaining([expect.stringContaining('Query parameter is required')])
                        );
                    });
            });
            ...
            it('when page is not a valid number (NaN)', () => {
                return request(app.getHttpServer())
                    ...
                    .expect((res) => {
                        expect(Array.isArray(res.body.message) ? res.body.message : [res.body.message]).toEqual(
                            expect.arrayContaining([expect.stringContaining('Page must be higher than zero')])
                        );
                    });
            });
            ...
            it('page is a decimal number', () => {
                return request(app.getHttpServer())
                    ...
                    .expect((res) => {
                        expect(Array.isArray(res.body.message) ? res.body.message : [res.body.message]).toEqual(
                            expect.arrayContaining([expect.stringContaining('Page must be a valid number')])
                        );
                    });
            });
        });
